...
function IPSSAssessment({ onComplete }) {
  ...
  const handleCompleteInternal = () => {
    const severity = getSeverity(totalScore);
    const r = { score: totalScore, severity, qol: qolAnswer, date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) };
    setResult(r);
    onComplete(r);
    setPhase("result");
  };
    ...
    <div className="page">
      ...
      <div className="actions-row">
        ...
        {phase === "questions" ? (
          ...
          <button className="btn btn-primary" onClick={handleCompleteInternal} disabled={!qolAnswer}>
            View Results →
          </button>
        )}
      </div>
...
function AuthPage({ onLogin }) {
  ...
  const handleSubmit = () => {
    onLogin({ name: form.name || "Dr. Patient", email: form.email || "patient@hospital.org" });
  };
      ...
      <div className="app-shell">
        <aside className="sidebar">
          ...
          <div className="sidebar-footer">
            ...
            <button onClick={() => setAuthed(false)} style={{ marginTop: 10, fontSize: 11, color: "rgba(255,255,255,0.4)", background: "none", border: "none", cursor: "pointer", padding: 0 }}>Sign out</button>
          </div>
        </aside>
